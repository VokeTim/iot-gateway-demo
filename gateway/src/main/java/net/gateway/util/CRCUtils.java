package net.gateway.util;

/**
 * CRC校验工具类
 */
public class CRCUtils {

    // Table of CRC values for high-order byte
    private final static short[] lookupCRCHi = {  };

    // Table of CRC values for low-order byte
    private final static short[] lookupCRCLo = {  };




    public static boolean isValidCRC(byte[] src) {
        byte[] data = subbytes(src, 0, src.length - 2);
        byte[] crc = subbytes(src, src.length -2);
        assert data != null;
        byte[] calculated = calculateCRC(data);
        return  (crc[0] << 8 | crc[1]) == (calculated[0] << 8 | calculated[1]);
    }

    public static byte[] calculateCRC(byte[] data) {
        int high = 0xFF, low = 0xFF;
        int nextByte = 0;
        int uIndex; /* will index into CRC lookup */ /* table */
        /* pass through message buffer */
        for (int i = 0; i < data.length; i++) {
            nextByte = 0xFF & ((int) data[i]);
            uIndex = high ^ nextByte; // *puchMsg++; /* calculate the CRC */
            high = low ^ lookupCRCHi[uIndex];
            low = lookupCRCLo[uIndex];
        }
        //高低位反转
        //return (low << 8) | high;
        //return (high << 8) | low;
        byte[] ret = new byte[2];
        ret[0] = (byte) (high&0xFF);
        ret[1] = (byte) (low&0xFF);
        return ret;
    }

    /**
     * Returns a byte[] that is a sub of this src.
     *
     * @param src    the source array.
     * @param srcPos starting position in the source array.
     * @param length the number of array elements to be copied.
     */
    public static byte[] subbytes(byte[] src, int srcPos, int length) {
        if (length <= 0) {
            return null;
        }
        byte[] data = new byte[length];
        System.arraycopy(src, srcPos, data, 0, length);
        return data;
    }

    /**
     * Returns a byte[] that is a sub of this src.
     *
     * @param src    the source array.
     * @param srcPos starting position in the source array.
     */
    public static byte[] subbytes(byte[] src, int srcPos) {
        if (srcPos < 0 || srcPos > src.length) {
            return null;
        }
        byte[] data = new byte[src.length - srcPos];

        System.arraycopy(src, srcPos, data, 0, src.length - srcPos);

        return data;
    }
}
